...
const dirSetup = require("../settings.json").applicationCommandDirs;
const { loadSlashGlobal } = require("../settings.json")
		...
		client.on("ready", () => {
			if(loadSlashGlobal){
				...
				.then(slashCommandsData => {
					console.log(`${slashCommandsData.size} slashCommands ${`(With ${slashCommandsData.map(d => d.options).flat().length} Subcommands)`.red} Loaded for all: ${`All possible Guilds`}`.brightRed); 
					console.log(`Because u are Using Global Settings, it can take up to 1 hour until the Commands are changed!`.bold.green)
				...
				client.guilds.cache.map(g => g).forEach((guild) => {
					try{
						...
						.then(slashCommandsData => {
							console.log(`${slashCommandsData.size} slashCommands ${`(With ${slashCommandsData.map(d => d.options).flat().length} Subcommands)`.red} Loaded for: ${`${guild.name}`}`.brightRed); 
						}).catch((e)=>console.log(e));
					}catch (e){
